    ...
    public function up(): void
{
    Schema::table('fuel_records', function (Blueprint $table) {
        ...
        $table->decimal('current_weight', 10, 2)->nullable()->change();
        $table->decimal('max_load', 10, 2)->nullable()->change();
    });

    if (! Schema::hasColumn('fuel_records', 'total_fuel_liters')) {
        Schema::table('fuel_records', function (Blueprint $table) {
            $table->decimal('total_fuel_liters', 10, 2)->nullable()->after('cost_fuel_total');
        });
    }

    if (! Schema::hasTable('fuel_record_segments')) {
        Schema::create('fuel_record_segments', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('fuel_record_id'); // ปรับตามชนิดของ id_fuel_record
            $table->unsignedInteger('sequence');
            $table->string('start_point', 255);
            $table->string('destination', 255);
            $table->decimal('load_weight', 10, 2)->default(0);
            ...
            $table->unique(['fuel_record_id', 'sequence']);
        });
    }
}
