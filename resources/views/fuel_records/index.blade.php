    ...
    <div class="container py-3">
        ...
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
            ...
            @if (in_array(auth()->user()->role, ['admin', 'staff'], true))
                <a href="{{ route('fuel_records.create') }}" class="btn btn-dark">
                    <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor" stroke-width="2" class="me-1" aria-hidden="true">
        ...
        <div class="fuel-filter-card p-3 shadow-sm mb-3">
            <form method="GET" action="{{ route('fuel_records.index') }}">
                <div class="row g-3 align-items-end">
                    <div class="col-lg-4 col-md-6">
                        ...
                        <input type="text" id="q" name="q" class="form-control"
                            placeholder="ต้นทาง ปลายทาง หรือทะเบียนรถ" value="{{ $q }}">
                    </div>

                    <div class="col-lg-3 col-md-6">
                        <label for="trucks_id" class="form-label">
                            ทะเบียนรถ
                        </label>

                        <select id="trucks_id" name="trucks_id" class="form-select">
                            <option value="">
                                ทะเบียนทั้งหมด
                            </option>

                            @foreach ($trucks as $truck)
                                <option value="{{ $truck->id_truck }}" @selected($trucks_id === (string) $truck->id_truck)>
                                    {{ $truck->id_truck }}
                    ...
                    <div class="col-lg-1 col-md-6">
                        <button type="submit" class="btn btn-primary w-100" title="ค้นหา" aria-label="ค้นหา">
                ...
                @if ($q !== '' || $trucks_id !== '' || $date_from || $date_to)
                    <div class="mt-3">
                        <a href="{{ route('fuel_records.index') }}" class="btn btn-outline-secondary btn-sm">
                            ล้างตัวกรอง
                        </a>
                    </div>
                @endif
            ...
            <table class="table table-hover align-middle mb-0 fuel-table">
                ...
                <tbody>
                    @forelse ($records as $record)
                        <tr>
                            <td>
                                {{ $record->date_record ? $record->date_record->format('d/m/Y') : '-' }}
                            </td>
                            ...
                            <td>
                                ...
                                <small class="text-muted">
                                    {{ $record->truck?->brand?->name_brand ?? ($record->truck?->province_truck ?? '-') }}
                                </small>
                            </td>

                            <td>
                                <div class="fuel-location" title="{{ $record->start_point }}">
                                    {{ $record->start_point }}
                                </div>
                            </td>

                            <td>
                                <div class="fuel-location" title="{{ $record->destination }}">
                                    {{ $record->destination }}
                                </div>
                            </td>
                            ...
                            <td>
                                <div class="d-flex justify-content-center gap-2 flex-nowrap">
                                    <a href="{{ route('fuel_records.show', $record->id_fuel_record) }}"
                                        class="btn btn-outline-info fuel-action-button" title="ดูรายละเอียด"
                                    ...
                                    @if (in_array(auth()->user()->role, ['admin', 'staff'], true))
                                        <a href="{{ route('fuel_records.edit', $record->id_fuel_record) }}"
                                            class="btn btn-outline-primary fuel-action-button" title="แก้ไข"
                                            aria-label="แก้ไข">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke="currentColor" stroke-width="2" aria-hidden="true">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        <form method="POST"
                                            action="{{ route('fuel_records.destroy', $record->id_fuel_record) }}"
                                            class="m-0" data-confirm="ต้องการลบข้อมูลบันทึกน้ำมันรายการนี้หรือไม่"
                                            data-confirm-title="ยืนยันการลบข้อมูล" data-confirm-variant="danger"
                                            data-confirm-ok="ลบข้อมูล">
                                            @csrf
                                            @method('DELETE')

                                            <button type="submit" class="btn btn-outline-danger fuel-action-button"
