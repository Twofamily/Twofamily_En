        ...
        <form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="name_product" value="{{ old('name_product') }}"
                    class="form-control @error('name_product') is-invalid @enderror" placeholder="เช่น หินกรวด" required>
                @error('name_product')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="detail_product" class="form-control @error('detail_product') is-invalid @enderror" rows="3"
                    placeholder="เช่น ใช้สำหรับงานก่อสร้าง หรือถนน">{{ old('detail_product') }}</textarea>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="unit_price" value="{{ old('unit_price') }}"
                    class="form-control @error('unit_price') is-invalid @enderror" placeholder="เช่น 390" min="0" required>
                @error('unit_price')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="product_type_id" class="form-select @error('product_type_id') is-invalid @enderror">
                    ...
                    @foreach ($types as $t)
                        <option value="{{ $t->id_product_type }}" @selected(old('product_type_id') == $t->id_product_type)>
                            {{ $t->name_product_type }}
                        </option>
                    @endforeach
                </select>
